function handleBuyProduct(){
    ...
    html += `<p class="card-text text-dark fs-16 price-modal" data-price=${price}>${price}</p>`;
    html += `</div>`;
...
function handleClickQuantity(){
    $('.btn-number').click(function(){
        ...
        let price = $('.price-modal').text().replace(' đ','');
        let priceOriginal = $('.price-modal').data('price');
        if(type === 'minus'){
            if(quantity <= 1){
                $('.input-number').val(1)
                $('.price-modal').text(parseInt(priceOriginal) + ' đ');
            }else{
                let minus = parseInt(quantity) - 1;
                $('.input-number').val(minus);
                $('.price-modal').text((parseInt(price) - parseInt(priceOriginal)) + ' đ');
            }
        }else if(type === 'plus'){
            if(quantity > 99){
                $('.input-number').val(99)
                $('.price-modal').text(parseInt(priceOriginal * 99) + ' đ');
            }else{
    ...
    $('.input-number').change(function(){
        ...
        if(quantity < 1){
            ...
            $('.price-modal').text(parseInt(quantity) * parseInt(priceOriginal) + ' đ');
        }
    })
}